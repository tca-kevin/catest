...
function manage_scripts_and_styles_in_head_tag()
{
	if (defined('VITE_DEV') && VITE_DEV) {
		...
		wp_enqueue_script('hmr-script', "https://catest.test:5173/src/testo.js", array(), null);

		wp_enqueue_style('hmr-style', "https://catest.test:5173/src/style.scss", array(), null);
	} else {
		wp_enqueue_script('theme-script', get_template_directory_uri() . '/testo.js', array(), wp_get_theme()->get('Version'));
